...
class DataValidation:
    ...
    def detect_dataset_drift(self, reference: DataFrame, production: DataFrame) -> None:
        ...
        """
        try:
            data_drift_profile = Report(metrics=[DataDriftPreset(num_stattest='ks', cat_stattest='psi', 
                                                                num_stattest_threshold=0.2, 
                                                                cat_stattest_threshold=0.2)])
            data_drift_profile.run(reference_data=reference, current_data=production, column_mapping=None)
